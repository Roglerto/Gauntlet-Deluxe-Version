...
ModuleLevel5::ModuleLevel5(bool start_enabled) : Module(start_enabled)
{
	background = NULL;
	stars = NULL;
...
bool ModuleLevel5::readmap(){
	...
	myfilemap.open("Gauntlet/lvl5.txt", std::fstream::in);
...
update_status ModuleLevel5::PreUpdate(){
	...
	for (int x = 0; x < map.size(); x++){
		...
		if (map[x] == '1'){
			...
			if (map[x] == 'g') {
				...
				map[x] = 0;
				App->scene_items->additems(EXIT6, posmapelement.x, posmapelement.y);
			}
			else if (map[x] == 'i'){
...
update_status ModuleLevel5::PostUpdate(){
...
void ModuleLevel5::OnCollision(Collider* c1, Collider* c2)
{
